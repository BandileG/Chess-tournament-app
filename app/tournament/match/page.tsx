...
const TOURNAMENTS = [
  { id: 1, name: 'Blitzstake Starter', entry: 2, prize: 16, tier: 'Any', tierColor: 'text-gray-400', starts: 300 },
  { id: 2, name: 'Blitzstake Gambit', entry: 5, prize: 40, tier: 'Beginner+', tierColor: 'text-green-400', starts: 600 },
  { id: 3, name: 'Blitzstake Premium', entry: 8, prize: 64, tier: 'Intermediate+', tierColor: 'text-blue-400', starts: 900 },
  { id: 4, name: 'Blitzstake King', entry: 10, prize: 80, tier: 'Pro+', tierColor: 'text-purple-400', starts: 1200 },
  { id: 5, name: 'Blitzstake Emperor', entry: 15, prize: 120, tier: 'Legend+', tierColor: 'text-yellow-400', starts: 1500 },
  { id: 6, name: 'Blitzstake Grandmaster', entry: 35, prize: 280, tier: 'Grandmonster', tierColor: 'text-red-400', starts: 1800 },
]
...
export default function DashboardPage() {
  ...
  const [tournamentPlayers, setTournamentPlayers] = useState<Record<string, number>>({})
  ...
  useEffect(() => {
    ...
    supabase.auth.getUser().then(({ data: { user } }) => {
      ...
      supabase
        ...
        .then(({ data }) => {
          if (data?.id) {
            router.push(`/tournament/match?match_id=${data.id}`)
          }
        })
    })

    // Load real player counts from Supabase
    supabase
      .from('tournaments')
      .select('name, current_players, status')
      .eq('status', 'open')
      .then(({ data }) => {
        if (data) {
          const counts: Record<string, number> = {}
          data.forEach(t => { counts[t.name] = t.current_players })
          setTournamentPlayers(counts)
        }
      })
    ...
    const channel = supabase
      ...
      .on('postgres_changes', {
        ...
        if (updated.wallet_balance != null) setBalance(updated.wallet_balance)
      })
      .subscribe()
  ...
  const savedTier = typeof window !== 'undefined' ? localStorage.getItem('onboarding_tier') : null
  useEffect(() => {
        ...
        <div className="flex flex-col gap-4">
          {TOURNAMENTS.map(t => (
            <TournamentCard
              ...
              balance={balance}
              userId={userId}
              realPlayerCount={tournamentPlayers[t.name] ?? 0}
            />
          ))}
        </div>
      </div>
...
function TournamentCard({ tournament: t, balance, userId, realPlayerCount }: {
  ...
  const filled = realPlayerCount
  const spotsLeft = 10 - filled
  const fillPercent = Math.round((filled / 10) * 100)
  ...
  const handleJoin = async () => {
    ...
    try {
      ...
      // If match already exists go straight to match
      if (data.match_id) {
        router.push(`/tournament/match?match_id=${data.match_id}`)
        return
      }
    ...
    <div className="bg-[#0d1117] border border-[#1e2d3d] rounded-2xl p-5">
      ...
      <div className="mb-3">
        <div className="flex justify-between mb-1">
          <p className="text-gray-500 text-xs">{filled}/10 players</p>
